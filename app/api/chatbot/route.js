...
import { NextResponse } from "next/server";
import Fuse from "fuse.js";
import embeddingsData from "../../../lib/embeddings.json"; // Adjust path based on your project structure
import { pipeline } from "@xenova/transformers";

// Cache embedder globally to reduce cold start time
...
const initializeEmbedder = async () => {
  if (!embedderPromise) {
    embedderPromise = pipeline(
      ...
      "Xenova/all-MiniLM-L6-v2",
      {
        executionProviders: ["wasm"], // Force WebAssembly to avoid native bindings
      }
    );
  }
